...
class CounterpartyRiskAgent:
    ...
    def run(
        ...
        return {
            ...
            "ead_direction": [
                {
                    "filing_id": a.get("filing_id"),
                    "direction": a.get("ead_delta_direction"),
                }
                for a in analyses
                if a.get("ead_delta_direction") not in (None, "neutral")
            ],
            "xva_recalibrations": [
                a for a in analyses if a.get("xva_recalibration_required")
            ],
    ...
    def ead_reducing_opportunities(self) -> list[dict]:
        ...
        return [
            a
            for a in analyses
            if a.get("ead_delta_direction") in ("decrease", "reduction")
        ]
    ...
    def _analyze_ccr_filings(self) -> list[dict]:
        ...
        for rec in records:
            ...
            if filing.get("domain", "") not in CCR_DOMAINS:
                continue
            result = self.runtime.execute_skill(
                "counterparty_risk_analysis", {"filing": filing}
            )
            if result.success:
