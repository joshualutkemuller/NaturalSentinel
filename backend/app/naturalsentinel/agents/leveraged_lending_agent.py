...
class LeveragedLendingAgent:
    ...
    def run(
        ...
        return {
            ...
            "covenant_changes": _extract_field(
                analyses, "covenant_requirement_changes"
            ),
            "definition_changes": _extract_field(analyses, "definition_changes"),
            "supervisory_triggers": triggers,
            "portfolio_at_risk": _assess_portfolio_risk(
                triggers, portfolio_leverage_ratio
            ),
            "credit_policy_updates": _extract_field(
                analyses, "credit_policy_updates_required"
            ),
            "documentation_requirements": _extract_field(
                analyses, "documentation_requirements"
            ),
    ...
    def new_supervisory_review_threshold(self) -> float | None:
        ...
        for a in analyses:
            for t in a.get("leverage_threshold_changes", []):
                try:
                    thresholds.append(
                        float(str(t.get("new_threshold", "")).replace("x", ""))
                    )
    ...
    def _analyze_ll_filings(self) -> list[dict]:
        ...
        for rec in records:
            ...
            if filing.get("domain", "") not in LL_DOMAINS:
                continue
            result = self.runtime.execute_skill(
                "leveraged_lending_assessment", {"filing": filing}
            )
            if result.success:
