...
class SecuritiesFinancingAgent:
    ...
    def run(
        ...
        return {
            ...
            "collateral_changes": _extract_field(
                analyses, "collateral_eligibility_changes"
            ),
    ...
    def prime_brokerage_impacts(self) -> list[dict]:
        ...
        return [
            {
                "filing_id": a.get("filing_id"),
                "impacts": a.get("prime_brokerage_impacts", []),
            }
            for a in analyses
    ...
    def _analyze_sf_filings(self) -> list[dict]:
        ...
        for rec in records:
            ...
            if filing.get("domain", "") not in SF_DOMAINS:
                continue
            result = self.runtime.execute_skill(
                "securities_financing_analysis", {"filing": filing}
            )
            if result.success:
