...
from app.naturalsentinel.cli import build_provider
from app.naturalsentinel.fetchers import DOMAIN_BUSINESS_LINES, fetch_filings
from app.naturalsentinel.fetchers.sample_data import MOCK_ANALYSES, SAMPLE_FILINGS
...
class TestSerialization(unittest.TestCase):
    def _make_result(self):
        ...
        return MonitorResult(
            filing=f, impact=i, decision=d, evidence_ledger=[e], raw_analysis="{}"
        )
...
class TestMemoryStore(unittest.TestCase):
    ...
    def test_filing_history(self):
        self.mem.store_episodic(
            "SEC-001", {"title": "A", "domain": "sec"}, {"severity": "h"}
        )
        self.mem.store_episodic(
            "FDA-001", {"title": "B", "domain": "fda"}, {"severity": "l"}
        )
    ...
    def test_context_block_with_precedents(self):
        self.mem.store_episodic(
            "T-001", {"title": "A", "domain": "sec"}, {"severity": "med"}
        )
...
class TestAgent(unittest.TestCase):
    ...
    def test_reset_state(self):
        ...
        self.agent.reset_state()
        self.assertEqual(
            len(self.agent.run(since_days=90)), len(fetch_filings(since_days=90))
        )
    ...
    def test_memory_populated(self):
        self.agent.run(since_days=90)
        self.assertEqual(
            self.mem.count(MemoryType.EPISODIC), len(fetch_filings(since_days=90))
        )
