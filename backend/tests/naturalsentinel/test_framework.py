...
from app.naturalsentinel import (
    FULL,
    READONLY,
    STANDARD,
    AgentRuntime,
...
from app.naturalsentinel.fetchers import fetch_filings
...
class TestLegacyRuntimeParity(unittest.TestCase):
    ...
    def test_scan_cycle_matches_legacy_agent_outputs(self):
        ...
        runtime_by_id = {
            item["filing"]["id"]: item for item in runtime_result.data["results"]
        }

        self.assertEqual(set(agent_by_id), set(runtime_by_id))
        self.assertEqual(
            len(agent_results), runtime_result.data["stats"]["new_analyzed"]
        )

        for filing_id, agent_result in agent_by_id.items():
            runtime_item = runtime_by_id[filing_id]
            self.assertEqual(
                runtime_item["impact"]["severity"], agent_result.impact.severity.value
            )
...
class TestFrameworkInvariants(unittest.TestCase):
    ...
    def test_dependency_failure_propagates_and_is_linked_in_audit(self):
        ...
        self.assertEqual(
            child_entries[0].parent_invocation_id, parent_entries[0].invocation_id
        )
...
class TestStableFixtureSubset(unittest.TestCase):
    ...
    def test_fixture_subset_exact_mock_severities(self):
        runtime = AgentRuntime(
            provider=MockProvider(), policy=SecurityPolicy(allowed=FULL)
        )
        runtime.register_skills(*ALL_SKILLS)
    ...
    def test_fixture_subset_dedup_counts_are_exact(self):
        ...
        first = runtime.execute_skill(
            "detect_duplicates", {"filings": subset, "mark_seen": True}
        )
        second = runtime.execute_skill(
            "detect_duplicates", {"filings": subset, "mark_seen": True}
        )
...
class TestMemorySkills(unittest.TestCase):
    def setUp(self):
        self.memory = MemoryStore(":memory:")
        self.runtime = AgentRuntime(
            memory=self.memory, policy=SecurityPolicy(allowed=FULL)
        )
        self.runtime.register_skills(*ALL_SKILLS)
...
class TestScanCycleSkill(unittest.TestCase):
    ...
    def test_second_cycle_deduped(self):
        ...
        self.assertEqual(
            r2.data["stats"].get("new_analyzed", r2.data["stats"].get("new", 0)), 0
        )
    ...
    def test_domain_filter(self):
        result = self.runtime.execute_skill(
            "scan_cycle", {"domains": ["sec"], "since_days": 90}
        )
        self.assertTrue(result.success)
