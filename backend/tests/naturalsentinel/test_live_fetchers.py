...
from app.naturalsentinel.fetchers.base import _raw_to_filing, fetch_filings
...
class TestDetectChangeType(unittest.TestCase):
    def test_final_rule(self):
        self.assertEqual(
            detect_change_type("Final Rule on Capital Requirements"), "final_rule"
        )

    def test_proposed_rule(self):
        self.assertEqual(
            detect_change_type("Notice of Proposed Rulemaking for NSFR"),
            "proposed_rule",
        )

    def test_nprm(self):
        self.assertEqual(
            detect_change_type("NPRM on climate disclosures"), "proposed_rule"
        )

    def test_enforcement(self):
        self.assertEqual(
            detect_change_type("Enforcement action and civil money penalty"),
            "enforcement",
        )

    def test_guidance(self):
        self.assertEqual(
            detect_change_type("Supervisory guidance on model risk"), "guidance"
        )

    def test_notice_default(self):
        self.assertEqual(
            detect_change_type("Quarterly report from the agency"), "notice"
        )
...
class TestFederalRegisterFetch(unittest.TestCase):
    ...
    def test_empty_results_list(self):
        client = MockHTTPClient(
            {"https://www.federalregister.gov/api/v1": {"results": []}}
        )
        results = federal_register.fetch(
            ["fed"], since_days=30, fetch_full_text=False, client=client
        )
...
class TestEdgarFetch(unittest.TestCase):
    ...
    def test_returns_filings(self):
        results = edgar.fetch(
            since_days=30, fetch_full_text=False, client=self._make_client()
        )
...
class TestBisFetch(unittest.TestCase):
    ...
    def test_returns_filings(self):
        results = bis.fetch(
            since_days=365, fetch_full_text=False, client=self._make_client()
        )
        self.assertGreater(len(results), 0)
    ...
    def test_cutoff_filters_old_publications(self):
        ...
        results = bis.fetch(
            since_days=1, fetch_full_text=False, client=self._make_client()
        )
...
class TestFetchFilingsLive(unittest.TestCase):
    ...
    def test_live_mode_returns_regulatory_filings(self):
        from app.naturalsentinel.models import RegulatoryDomain
