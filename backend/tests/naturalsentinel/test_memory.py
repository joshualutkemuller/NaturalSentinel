...
class TestMemoryStoreBasics:
    ...
    def test_record_feedback(self, memory):
        ...
        memory.record_feedback(
            "TEST-001", "severity", "medium", "high", "has enforcement risk"
        )
...
class TestMemoryRetrieval:
    def test_get_by_id(self, memory):
        memory.store_episodic(
            "TEST-001", {"title": "Climate Rule"}, {"severity": "high"}
        )
    ...
    def test_recall_with_type_filter(self, memory):
        memory.store_episodic(
            "T-001", {"title": "Test", "domain": "sec"}, {"severity": "high"}
        )
    ...
    def test_filing_history(self, memory):
        memory.store_episodic(
            "SEC-001", {"title": "A", "domain": "sec"}, {"severity": "high"}
        )
        memory.store_episodic(
            "FDA-001", {"title": "B", "domain": "fda"}, {"severity": "low"}
        )
        memory.store_episodic(
            "SEC-002", {"title": "C", "domain": "sec"}, {"severity": "medium"}
        )
...
class TestContextBlock:
    ...
    def test_context_includes_precedents(self, memory):
        memory.store_episodic(
            "T-001", {"title": "Test", "domain": "sec"}, {"severity": "medium"}
        )
