...
class SimilarityEngine:
    ...
    def index(self, documents: dict[str, str]):
        ...
        if self._use_sklearn and texts:
            ...
            self._doc_tokens = {
                did: _tokenize(text) for did, text in documents.items()
            }
...
class MemoryStore:
    ...
    def _upsert(self, record: MemoryRecord):
        ...
        """, (
            record.id, record.memory_type.value, record.namespace,
            record.key, json.dumps(record.content), record.embedding_text,
    ...
    def store_entity(self, entity_name: str, attributes: dict,
    ...
    def record_feedback(self, filing_id: str, field: str,
    ...
    def recall(self, query: str, top_k: int = 5,
               ...
               namespace: str | None = None) -> list[MemoryRecord]:
        """
    ...
    def get_filing_history(self, domain: str | None = None,
    ...
    def build_context_block(self, filing_domain: str, filing_text: str,
    ...
    def export_json(self) -> str:
        ...
        relations = [dict(r) for r in
                     self.conn.execute("SELECT * FROM entity_relations").fetchall()]
        feedback = [dict(r) for r in
                    self.conn.execute("SELECT * FROM feedback_log").fetchall()]
        return json.dumps({
            "memories": records,
            "relations": relations,
            "feedback": feedback,
