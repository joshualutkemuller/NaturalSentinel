...
Demonstrates:
  ...
  2. ComplianceTracker — deadline calendar with markdown report export
...
def _build_runtime() -> AgentRuntime:
    ...
    # Prime the memory with a full scan
...
def demo_alert_agent(runtime: AgentRuntime) -> None:
    ...
    # Check for HIGH+ alerts (no new scan, memory already primed)
    ...
    if trends:
        ...
        if trends.get("narrative"):
            _section("Trend Narrative (LLM)")
            # Print first 400 chars of narrative
            print(f"  {trends['narrative'][:400]}…")
...
def demo_trend_analysis(runtime: AgentRuntime) -> None:
    ...
    _section("Domain Activity")
    for dom, count in stats.get("domain_activity", {}).items():
        print(f"  {dom.upper():<6} {count} filing(s)")

    _section("Top Affected Business Lines")
    for bl in stats.get("top_affected_business_lines", [])[:5]:
        print(f"  {bl['count']}x  {bl['name']}")
...
def demo_cross_domain(runtime: AgentRuntime) -> None:
    ...
    if not result.success:
        print(f"  Cross-domain analysis failed: {result.error}")
        return
    ...
    print(f"  Min domains threshold:  {summary.get('min_domains_threshold', 2)}")

    if not intersections:
        print("\n  No multi-agency intersections found in current memory.")
        return
    ...
    for ix in intersections[:5]:
        ...
        if ix["combined_regulations"]:
            print(f"    Regs: {', '.join(ix['combined_regulations'][:3])}")
        print()

    if data.get("assessment"):
        _section("LLM Assessment (first 500 chars)")
        print(f"  {data['assessment'][:500]}…")
...
def demo_export_report(runtime: AgentRuntime) -> None:
    ...
    for line in lines[:8]:  # Header + first 7 data rows
        # Truncate long lines for display
        print(f"  {line[:100]}")
    if len(lines) > 8:
        print(f"  … ({len(lines) - 8} more rows) …")

    # Also show JSON format as a compact sample
    ...
    if json_result.success:
        _section("JSON Export Sample (high+ severity, 2 records)")
        import json as _json
        try:
            records = _json.loads(json_result.data["content"])
            for rec in records[:2]:
                print(f"  {rec.get('filing_id')} | {rec.get('severity').upper()} | {rec.get('title', '')[:50]}")
        except Exception:
            pass
