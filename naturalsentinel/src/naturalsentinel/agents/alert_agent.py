...
The agent classifies alerts into urgency tiers, aggregates by domain,
and surfaces the most actionable items first.
...
class AlertAgent:
    ...
    def __init__(self, runtime: Any):
        """
        Args:
            runtime: An AgentRuntime with alert_threshold, scan_cycle, and
                     trend_analysis skills registered.
        """
    ...
    def check(
        ...
        Args:
            threshold: Minimum severity to alert on ('low','medium','high','critical').
            limit: Max analyses to inspect.
            include_trends: Whether to also run trend_analysis.

        Returns:
            {
                "alerts":            list[dict],
                "summary":           dict,
                "immediate_actions": list[str],   # de-duplicated across all alerts
                "trends":            dict | None,
                "audit_summary":     dict,
            }
        """
    ...
    def scan_and_alert(
        ...
        Args:
            domains: Agency codes to scan (empty = all).
            since_days: Look-back window for the scan.
            threshold: Alert threshold applied after the scan.
            include_trends: Whether to include trend analysis.

        Returns:
            Same shape as check() plus:
                "scan_stats": dict  — stats from the scan cycle
        """
    ...
    def _run_alert_threshold(self, threshold: str, limit: int) -> dict:
        ...
        if result.success:
            return result.data
        return {"alerts": [], "summary": {"error": result.error}}
