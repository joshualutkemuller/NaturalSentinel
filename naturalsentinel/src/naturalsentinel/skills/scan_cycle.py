...
class ScanCycleSkill(Skill):
    metadata = SkillMetadata(
        ...
        version="1.0.0",
        permissions=(
            Permission.FETCH_LOCAL | Permission.LLM_READ | Permission.LLM_WRITE
            | Permission.MEMORY_READ | Permission.MEMORY_WRITE
            | Permission.STATE_READ | Permission.STATE_WRITE
        ),
        latency=LatencyClass.BATCH,
        parameters=[
            SkillParameter("domains", "list[str]", "Agency codes to scan.", required=False, default=[]),
    ...
    def execute(self, context: SkillContext) -> SkillResult:
        ...
        # Step 1: Fetch
        fetch_result = context.invoke_skill("fetch_filings", {
            "domains": domains, "since_days": since_days,
        })
        ...
        # Step 2: Deduplicate
        dedup_result = context.invoke_skill("detect_duplicates", {
            "filings": all_filings, "mark_seen": True,
        })
        ...
        for filing in new_filings:
            ...
            ctx_result = context.invoke_skill("build_context", {
            ...
            # 4. Analyze
            analyze_result = context.invoke_skill("analyze_filing", {
                "filing": filing,
            ...
            # 5. Store in memory
            context.invoke_skill("store_memory", {
