...
class OptimizationConstraintAgent:
    ...
    def run(
        ...
        return {
            ...
            "hard_constraints": [a for a in analyses if a.get("constraint_type") == "hard"],
            "soft_constraints": [a for a in analyses if a.get("constraint_type") == "soft"],
    ...
    def _analyze_all_filings(self, domains: list[str]) -> list[dict]:
        records = self.runtime.memory.get_filing_history(limit=100) if self.runtime.memory else []
        analyses = []
        for rec in records:
