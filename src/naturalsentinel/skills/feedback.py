...
class RecordFeedbackSkill(Skill):
    metadata = SkillMetadata(
        ...
        parameters=[
            ...
            SkillParameter("field", "str", "Which field to correct (severity, affected_business_lines, etc.)."),
            SkillParameter("old_value", "str", "The current/wrong value."),
            SkillParameter("new_value", "str", "The correct value."),
            SkillParameter("reason", "str", "Why this correction is needed.", required=False, default=""),
        ],
    ...
    def execute(self, context: SkillContext) -> SkillResult:
        ...
        p = context.params
        context.memory.record_feedback(p["filing_id"], p["field"], p["old_value"], p["new_value"], p.get("reason", ""))
        return SkillResult(
